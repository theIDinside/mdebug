  ...
  for (const auto &frame : cfs.frames) {
    if (frame.frame_type() == sym::FrameType::Full) {
      ...
      if (lt.is_valid()) {
        ...
        const auto src = full.symbol_info()->name();
...
Variables::execute(Tracer *tracer) noexcept
{
  ...
  auto current = tracer->get_current();
  if (const auto varref = current->var_ref(var_ref); varref) {
    auto frame = tracer->get_current()->frame(varref->frame_id);
    ObjectFile *obj = varref->objectfile();
    switch (varref->type) {
    ...
    case EntityType::Variable: {
      TODO("Variables request for member variables of a type not yet implemented");
