...
struct Field
{
  ...
  Immutable<SymbolLocation> offset_of;
...
public:
  ...
  serialize_type_name(Out out)
  {
    ...
    if (ordered[i]->modifier != Modifier::None) {
      o = std::format_to(o, "{}", ModifierToString(ordered[i]->modifier));
    } else {
      o = std::format_to(o, "{}", *ordered[i]->name);
    }
    ++i;
  ...
  typename_to_str()
  {
    ...
    auto i = 0;
    if (ordered[i]->modifier != Modifier::None) {
      out_iter = std::format_to(out_iter, "{}", ModifierToString(ordered[i]->modifier));
    } else {
      out_iter = std::format_to(out_iter, "{}", *ordered[i]->name);
    }
    ++i;
...
public:
  ...
  // anythign else.
  static std::shared_ptr<Value> pull_value(TraceeController *tc, NonNullPtr<TaskInfo> task,
...
format_primitive_to(Type &type, const Bytes &span, OutBuf outbuffer) noexcept
{
  ...
  if (type.is_reference()) {
    ...
    return std::format_to(type_name_serialized, " (0x{:x})", ptr);
  }
  ...
  case BaseTypeEncoding::DW_ATE_address: {
    ...
    return std::format_to(outbuffer, "0x{}", value);
  }
  case BaseTypeEncoding::DW_ATE_boolean: {
    bool value = bit_copy<bool>(span);
    return std::format_to(outbuffer, "{}", value);
  }
