...
FunctionSymbol::FunctionSymbol(AddrPtr start, AddrPtr end, std::string_view name, std::string_view member_of,
    ...
    : decl_file(decl_file), fully_parsed(false), is_member_fn(false), formal_parameters(start, end, {}),
      function_body_variables(), maybe_origin_dies(maybe_origin), framebase_expr(fb_expr), pc_start(start),
...
{
  return formal_parameters.symbols;
}
