...
Value::Value(std::string_view member_name, Field &kind, u32 mem_contents_offset,
             std::shared_ptr<MemoryContentsObject> value_object) noexcept
    : name(member_name), mem_contents_offset(mem_contents_offset), value_origin(&kind),
      value_object(std::move(value_object))
{
}
                                            ...
                                            NonNullPtr<sym::Frame> frame, Symbol &symbol) noexcept
{
  ...
  case LocKind::DwarfExpression: {
    ...
    auto bytes = tc.read_to_vec(address, requested_byte_size);
    auto memory_object = std::shared_ptr<MemoryContentsObject>(
        new MemoryContentsObject{address, address + requested_byte_size, std::move(bytes)});
    return std::shared_ptr<Value>(new Value{symbol.name, symbol, 0, memory_object});
  }
