...
namespace mdb::js {
void
CompileBreakpointCallable::trace(JSTracer *trc, const char *name)
{
  JS::TraceEdge(trc, &mCompiledFunction, "breakpoint condition");
}

std::optional<EventResult>
CompileBreakpointCallable::EvaluateCondition(JSContext *cx, mdb::TaskInfo *task, mdb::UserBreakpoint *bp) noexcept
{
...
CompileBreakpointCallable::EvaluateLog(JSContext *cx, mdb::TaskInfo *task, mdb::UserBreakpoint *bp) noexcept
{
  JS::Rooted<JSFunction *> fn{cx, mCompiledFunction};
  JS::RootedValue rval(cx);
  ...
  JS::HandleFunction f{fn};
  std::string err;
  auto r = Tracer::GetScriptingInstance().CallFunction<std::string>(f, args, err);
  if (!r) {
    DBGLOG(interpreter, "CallFunction failure: {}", err);
    return {};
  }
