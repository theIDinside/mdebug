...
ScopedFd::FileSize() const noexcept
{
  ...
  MDB_ASSERT(-1 != curr, "Failed to fseek");
  auto size = lseek(mFd, 0, SEEK_END);
  MDB_ASSERT((off_t)-1 != size, "Failed to get size");
...
{
  MDB_ASSERT(fs::exists(p), "File did not exist {}", p.c_str());
...
ScopedFd::OpenSocketConnectTo(const std::string &host, int port) noexcept
{
  ...
  if (getaddrinfo(host.c_str(), port_number.c_str(), &hints, &result) != 0) {
    DBGLOG(core,
      "getaddrinfo failed when attempting to connect to {}:{}. Reported reason: {}",
      host,
      port,
      strerror(errno));
  ...
  if (socket_error_only) {
    DBGLOG(
      core, "Failed to connect to {}:{} due to socket error. Reported reason: {}", host, port, strerror(errno));
