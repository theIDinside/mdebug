...
template <> struct formatter<CoreEventType>
{
  ...
  format(const CoreEventType &evt, FormatContext &ctx) const
  {
    ...
    case CoreEventType::DeferToProceed:
...
// action of a task
struct DeferToProceed : public ThreadEvent
{
  EventType(DeferToProceed);
  bool attached;
};
...
using CoreEventVariant =
    std::variant<WatchpointEvent, SyscallEvent, ThreadCreated, ThreadExited, BreakpointHitEvent, Fork, Clone, Exec,
                 ProcessExited, LibraryEvent, Signal, Stepped, DeferToProceed>;
...
struct CoreEvent
{
  ...
  // The signal generated (or the exit code returned) by the process that generated the event
  union
  {
    int signal;
    int exit_code;
  ...
  CoreEvent(Pid target, Tid tid, CoreEventVariant &&p, int sig_code, RegisterData &&regs) noexcept;
  CoreEvent(const EventDataParam &param, CoreEventVariant &&p, RegisterData &&regs) noexcept;
  ...
  static CoreEvent *ThreadCreated(const EventDataParam &param, RegisterData &&reg) noexcept;
  static CoreEvent *ThreadExited(const EventDataParam &param, RegisterData &&reg) noexcept;
  ...
  static CoreEvent *Stepped(const EventDataParam &param, bool stop, RegisterData &&reg) noexcept;
  static CoreEvent *DeferToProceed(const EventDataParam &param, RegisterData &&reg, bool attached) noexcept;
};
