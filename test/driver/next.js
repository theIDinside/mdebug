...
async function test() {
  ...
  if (evt.reason != "step") {
    throw new Error(`Expected to see a 'stopped' event with 'step' as reason. Got event ${JSON.stringify(evt)}`);
  }
  ...
  if (next_pc != disassembly.body.instructions[1].address) {
    throw new Error(`Expected to be at ${disassembly.body.instructions[1].address} but RIP=${next_pc} (previos pc: ${pc})`);
  }
}
