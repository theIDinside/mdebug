const { DAClient, MDB_PATH, buildDirFile, checkResponse, getLineOf, readFile, repoDirFile, runTestSuite } =
  require('./client')(__filename)

async function set4Breakpoints() {
  const da_client = new DAClient(MDB_PATH, [])
  // we don't care for initialize, that's tested elsewhere
  const init = await da_client.sendReqGetResponse('initialize', {})
  checkResponse(init, 'initialize', true)
  const launch = await da_client.sendReqGetResponse('launch', {
  ...
  const bp_lines = ['BP1', 'BP2', 'BP3', 'BP4']
    ...
    .map((l) => ({ line: l }))
  const res = await da_client.sendReqGetResponse(bpRequest, {
    source: {
  ...
  if (res.body.breakpoints.length != 4) {
    throw new Error(`Expected bkpts 3 but got ${res.body.breakpoints.length}`)
  }
  ...
  if (found_all.some((v) => v == false)) {
    throw new Error(
      `Expected to get breakpoints for lines ${JSON.stringify(bp_lines)} but got ${JSON.stringify(
