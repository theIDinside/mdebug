...
async function baseTypes() {
  ...
  const bp_lines = ["LOC_BP", "ARGS_BP", "STRUCT_BP", "BYVAL_BP"]
  ...
  if (frames[0].name != "locals") throw new Error(`Expected to be be in 'locals' frame but was at ${frames[0].name}`);
  const scopes_res = await da_client.sendReqGetResponse("scopes", { frameId: frames[0].id });
  const scopes = scopes_res.body.scopes;
  if (scopes.length != 3) throw new Error(`expected 3 scopes but got ${scopes.length}. Scopes response: ${prettyJson(scopes_res)}`);
