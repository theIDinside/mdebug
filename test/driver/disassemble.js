...
function processObjdumpLines(insts) {
  ...
  for (const line of insts) {
    ...
    if (rep != -1) {
      ...
      res[res.length - 1].opcode = `${res[res.length - 1].opcode} ${line
        .substring(pos + 2)
        .trimEnd()}`;
    }
  }
...
function compareDisassembly(pc, objdump, mdbResult) {
  if (mdbResult.length != objdump.length) {
    throw new Error(
      `(${pc}): Expected ${objdump.length
      } disassembled instructions but instead got ${mdbResult.length
      }. Serial data: ${JSON.stringify(
        mdbResult,
        null,
        2
      )}. Expected data ${JSON.stringify(objdump, null, 2)}`
    );
  }
  console.log(
    `MDB Instruction Output: ${mdbResult.length} == objdump Instruction Output: ${objdump.length}`
  );
  for (let i = 0; i < objdump.length; ++i) {
    if (mdbResult[i].address != objdump[i].addr) {
      const serial = JSON.stringify(mdbResult, null, 2);
      throw new Error(
        `(${pc}): Expected disassembled instruction #${i} at address ${objdump[i].addr
        } but got ${mdbResult[i].address
        }. Serial data: ${serial}. Expected data ${JSON.stringify(
          objdump,
          null,
          2
        )}`
    ...
    if (
      ...
      mdbResult[i].instructionBytes.split(" ").join("") !=
      objdump[i].opcode.split(" ").join("")
    ) {
      throw new Error(
        `(${pc}): Expected disassembled instruction to have opcode ${objdump[i].opcode
        } but got ${mdbResult[i].instructionBytes
        }. Serial data: ${JSON.stringify(
          mdbResult,
          null,
          2
...
async function disasm_verify(objdump, client, pc, insOffset, insCount) {
  ...
  console.log(
    `[offset: ${insOffset}, pc: ${pc}, count: ${insCount}]\n\t - MDB output == objdump output!`
  );
}
